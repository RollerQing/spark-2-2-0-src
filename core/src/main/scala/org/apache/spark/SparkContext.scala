...
class SparkContext(config: SparkConf) extends Logging {
  ...
  try {
    ...
    //TaskSchedulerImpl实例由SparkContext携带着传递给DAGScheduler之后，便可以启动了： start方法都做了什么呢？
    // 到TaskSchedulerImpl类去看 其实第一句就是taskScheduler.start()调用了backend.start()
...
object SparkContext extends Logging {
  ...
  private def createTaskScheduler(
    ...
    // When running locally, don't try to re-execute tasks on failure. //当在本地模式运行时，失败的task不再重试
