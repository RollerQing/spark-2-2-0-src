 ...
 *
 * TaskScheduler的实现类，从SparkContext调用源头追踪task调度器的调用链。
 * TaskSchedulerImpl对象什么时候构建的？
 * 调用链的入口在SparkContext类的createTaskScheduler方法，
 * 在createTaskScheduler方法中根据用户指定的运行模式（spark.master参数）构建TaskSchedulerImpl对象
 * 且立即调用了TaskSchedulerImpl的initialize方法进行初始化。
 */
private[spark] class TaskSchedulerImpl private[scheduler](
  ...
  //
  //start方法都做了什么呢？可以看出taskScheduler.start()调用了backend.start()
  override def start() {
    backend.start()
